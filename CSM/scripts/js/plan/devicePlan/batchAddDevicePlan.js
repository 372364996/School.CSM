$(document).ready(function () {
    ...
    if (deviceType < 0) {
        ...
        $("#addEquipmentType").attr("disabled", "disabled");
    }
    if (regionId < 0) {
        ...
        $("#addEventRegion").attr("disabled", "disabled");
    }
    if (inbuildingId > 0)
    {
        getDeviceInfo(deviceType, regionId, inbuildingId)//根据设备类型获取设备
    }
    else {
...
function getInfoCorrectness() {
    ...
    var result = "";
    var deviceNamenum = $("#addAlarmEquipment").find("option:selected");
    $.each(deviceNamenum, function (index, item_new) {
        if (result != "")
            result += ",";
        result += item_new.label;
    });
    var deviceName = result.split(",")
    {
    
        for (var i = 0; i < deviceId.length; i++) {
            var executionCycle = "*|*|*|?"
            var startTimeStr = timeOccupancy(devicePlanId, deviceId[i], deviceName[i], addCreateTime, executionCycle, "");
            if (startTimeStr != "") {
                alert(startTimeStr);
                checkflag = false
                loadingpopup.close();
            ...
            var endTimeStr = timeOccupancy(devicePlanId, deviceId[i], deviceName[i], addEndTime, executionCycle, "");
            if (endTimeStr != "") {
                alert(endTimeStr);
                checkflag = false
                loadingpopup.close();
            ...
            //验证时间被占用没
            var timeOccupy = timeOccupancy(devicePlanId, deviceId[i], deviceName[i], addCreateTime, executionCycle, addEndTime)
            if (timeOccupy != "") {
                alert(timeOccupy);
                checkflag = false
                loadingpopup.close();
...
function insetDevicePlan(devicePlanId) {
    ...
    $.ajax({
        url: "/Plan/batchAddDevicePlanData",
        type: "post",
        ...
        success: function (data) {
            if (data.status == 0) {
                ...
                alert("获取智能预案注册出现" + data.msg + "错误请联系管理员！");
            }
        },
        error: function (XMLHttpRequest, textStatus, errorThrown) {
            alert("获取智能预案注册出现" + XMLHttpRequest.status + "错误请联系管理员！")
        }
    });
}
...
//检测预案中时间是否被占用
function timeOccupancy(devicePlanId, deviceId, deviceName, addCreateTime, executionCycle, addEndTime) {
    var strReset = "";
    $.ajax({
        ...
        success: function (data) {
            if (data.status == 0) {
                if (data.msg == true) {
                    ...
                    if (addEndTime != "") {
                        strReset += "设备" + deviceName + "在你选的时间段" + addCreateTime + "~" + addEndTime + "下已经被以下时间段占用 ";
                    }
                    else {
                        strReset += "设备" + deviceName + "在你选择的时间" + addCreateTime + "已经被以下时间段占用 ";
                    }
...
//根据设备类型获取设备
function getDeviceInfo(deviceType, regionId, inbuildingId) {
    if (deviceType != null) {
        $.ajax({
            url: "/Plan/GetDeviceInfo",
            type: "post",
            data: "deviceType=" + deviceType + "&regionId=" + regionId + "&inbuildingId=" + inbuildingId,
            datatype: 'json',
            async: false,
            success: function (data) {
                if (data.hasOwnProperty('state')) {
